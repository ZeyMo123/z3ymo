...
// ─────────────────────────────────────────────────────────────────
// app/admin/(dashboard)/content/guides/[id]/page.tsx
//
// Edit an existing guide.
// 3-state: loading → error (with retry) → ContentItemEditor
// All 16 fields passed to ContentItemEditor as `initial` props.
// ─────────────────────────────────────────────────────────────────
...
export default function EditGuidePage() {
  ...
  function load() {
    ...
    supabase.from('content_items').select('*').eq('id', id).single()
      .then(({ data: d, error: e }) => {
        if (e || !d) { setErrMsg(e?.message ?? 'Guide not found.'); setState('error'); return }
  ...
  if (state === 'loading') return (
    <div className="min-h-screen bg-whisper dark:bg-void flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-sm text-void/40 dark:text-whisper/40">Loading guide…</p>
      </div>
    </div>
  )
  ...
  if (state === 'error') return (
    <div className="min-h-screen bg-whisper dark:bg-void flex items-center justify-center px-6">
      <div className="text-center">
        ...
        <h2 className="font-display font-bold text-xl text-void dark:text-whisper mb-2">Couldn't load guide</h2>
        ...
        <div className="flex gap-3 justify-center">
          ...
          <Link href="/admin/content/guides" className="px-5 py-2.5 rounded-full border border-void/12 dark:border-whisper/12 text-sm text-void/60 dark:text-whisper/60 hover:border-void/25 transition-colors">Back to guides</Link>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <ContentItemEditor
      ...
      initial={{
        ...
        cta_type:            data.cta_type            ?? 'consultation',
        cta_custom_headline: data.cta_custom_headline ?? '',
        cta_custom_body:     data.cta_custom_body     ?? '',
