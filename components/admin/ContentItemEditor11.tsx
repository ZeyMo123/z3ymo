...
const EMPTY: ContentDraft = {
  ...
  featured:false, published:false, cta_type:'consultation',
...
export default function ContentItemEditor({ type, itemId, initial }: ContentItemEditorProps) {
  ...
  const handleSave = useCallback(async (publish = false) => {
    ...
    try {
      const payload = { ...draft, type, published: publish || draft.published }
            ...
            <div className="rounded-2xl border border-void/8 dark:border-whisper/8 p-4">
              ...
              <div className="flex gap-2">
                {[{ v:false, l:'Draft' }, { v:true, l:'Published' }].map(opt => (
                  <button key={String(opt.v)} type="button" onClick={() => update('published', opt.v)}
                    className={['flex-1 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer border',
                      draft.published === opt.v
                        ? opt.v ? 'bg-emerald/10 border-emerald/25 text-emerald'
